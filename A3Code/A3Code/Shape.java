...
abstract class Shape {
    ...
    public Shape[] getPath() { return getPathToRoot(this,0);} //this means the object itself

    public Shape[] getPathToRoot(Shape aShape, int depth) {
      Shape[] returnShapes;
      if (aShape == null) {
        if(depth == 0) return null;
        else returnShapes = new Shape[depth]; //base case, if aShape.getParent == null means reach to the root
      }
      else {
        depth++;
